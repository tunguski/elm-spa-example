...
checkCard card player =
  if member card player.hand
  then
    if member card player.selection
    then
...
placeCombination round =
  let selection = Maybe.map (\p -> p.selection) (Dict.get round.actualPlayer round.players)
      updatedPlayers = Dict.update round.actualPlayer (Maybe.map removeSelectionFromHand) round.players
  in { round | 
...
executeUpdate update game =
  case update of
    UpdatePlayer playerUpdate ->
      { game | round = updatePlayersDict playerUpdate game.round
             , log = update :: game.log }
    UpdateRound roundUpdate -> 
      { game | round = roundUpdate game.round
             , log = update :: game.log }
