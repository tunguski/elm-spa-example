module TichuLogic exposing (HandParams, buildHandParams, declareGrandTichu, declareTichu, demandRank, exchangeCards, executeIfNoError, giveDragon, hand, okSucceed, orElse, pass, playHandAndUpdateRound, processingResultToTask, seeAllCards, updateAndReturnIf)
...
hand userName table round player cards =
    ...
    in
    Nothing
        |> orElse (List.all ((\b a -> hasCard a b) <| player) param.parsedCards) "Tried to play card you don't own"
        ...
        |> executeIfNoError
            (if param.isActualPlayer then
                ...
                else if param.isBomb && param.bombEnoughPower then
                    -- play, switch to next player and return ok
                    doPlayCards
...
playHandAndUpdateRound table round player param =
    case param.parsedCards of
        [ Dog ] ->
            { table
                | round =
                    { round | actualPlayer = modBy 4 (round.actualPlayer + 2) }
...
pass userName table round player =
    Nothing
        ...
        |> orElse
            (hasCard MahJong player
                || (not <| openDemandMatch round)
            )
            "You have demanded card"
