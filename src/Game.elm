module Game exposing (declareGrandTichu, declareTichu, demandRank, doWithTable, exchangeCards, gamePostRequest, gamePostRequestPart, gamesApiPart, getGame, getGameWithRetry, giveDragon, hand, pass, seeAllCards, testGame, updateAndReturnIf)
...
exchangeCards api =
    doWithTable
        (\session table round player ->
            TichuLogic.exchangeCards session.username
                table
                round
                player
...
gamePostRequest :
    (Result String (Maybe (Cmd msg)) -> msg)
    -> String
    -> Maybe (Cmd msg)
